...
// Fork note: bora released protocol 16 (bumped past upstream's 14->15 session
// control/observe changes); both lineages then independently minted protocol 18
// with different message sets. This merge unions both message sets, so the
// source is bumped to 19 to exceed both released 18s and cleanly reject stale
// clients on either lineage.
