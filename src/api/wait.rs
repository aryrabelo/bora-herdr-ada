...
use crate::api::schema::{
    ErrorBody, ErrorResponse, EventData, Method, Request, ResponseResult, SuccessResponse,
};
...
pub(super) fn wait_for_event(
    ...
    } = &params.match_event
    {
        ...
        let mut active = match crate::api::subscriptions::ActiveSubscription::new(
            subscription,
            &request_id,
            0,
            api_tx,
    ...
    // ActiveSubscription's bare event variants (e.g. Subscription::PaneResultReported {})
    // carry no per-field filter the way EventMatch does (no pane_id, etc.), so they
    // can't stand in for a generic events.wait match.
    //
    // Start at 0 so a result posted just before the wait (the common post-then-wait
    // orchestration order) is still observed from the buffer.
...
fn event_match_subscription(
    request_id: &str,
    match_event: crate::api::schema::EventMatch,
) -> Result<crate::api::schema::Subscription, ErrorResponse> {
    match match_event {
        crate::api::schema::EventMatch::PaneAgentStatusChanged {
            pane_id,
            agent_status,
        } => Ok(crate::api::schema::Subscription::PaneAgentStatusChanged {
            pane_id,
...
fn wait_matched_response(request_id: &str, event: serde_json::Value) -> String {
    let Ok(event) = serde_json::from_value::<crate::api::schema::SubscriptionEventEnvelope>(event)
    else {
    ...
    let crate::api::schema::SubscriptionEventData::PaneAgentStatusChanged(data) = event.data else {
        return serde_json::to_string(&ErrorResponse {
            id: request_id.into(),
            error: ErrorBody {
                code: "unsupported_event_wait_match".into(),
