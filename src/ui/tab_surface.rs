...
mod tests {
    ...
    async fn desktop_full_app_semantic_frame_is_characterized() {
        ...
        // Golden updated 2026-09-08 for the removal of the right panel
        // (ceo-bora#269/#271): every visible symbol and every cell's fg/bg/
        // modifier over the full 106x20 frame are byte-identical to the
        // pre-removal golden (verified cell-by-cell against the
        // pre-#271 build, not just eyeballed) except exactly ONE cell —
        // row 19, column 105, the frame's bottom-right corner:
        //
        //   before  r19c105 sym="«" fg=40661126 bg=0 mod=0
        //   after   r19c105 sym="┘" fg=42579194 bg=0 mod=0
        //
        // That `«` was `render_right_panel_collapsed_toggle`'s affordance
        // glyph, pinned to the terminal's trailing edge when the (always
        // collapsed-by-default) right panel was present; it painted over
        // the split pane's own bottom-right border corner. With the right
        // panel gone, that cell now correctly shows the pane border's `┘`
        // instead — the intended, structural consequence of the removal,
        // not a regression. Every other assertion in this test still
        // holds, and the mobile characterization below is unchanged (the
        // mobile switcher never touched the right panel).
        assert_eq!(
            frame_digest(&frame),
            "75ac84022fa669dd4d83d335d549f436b6f2887ee0ddccff6dadc04045aa7279"
        );
    }
