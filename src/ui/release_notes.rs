...
mod tests {
    ...
    fn release_notes_preview_lines_show_update_steps() {
        let palette = Palette::catppuccin();
        let lines = release_notes_preview_line_entries("bora update", &palette)
            .into_iter()
            .map(|(_, line)| line)
            .collect::<Vec<_>>();
        ...
        assert_eq!(lines[1].spans[2].content.as_ref(), "bora update");
    ...
    fn release_notes_preview_display_is_part_of_the_scrollable_notes_body() {
        ...
        let lines = release_notes_display_lines(&notes, "bora update", &palette);
