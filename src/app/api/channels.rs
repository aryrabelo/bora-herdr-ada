...
/// recorded scope entry got a formatted suffix naming its write/read
/// directories, built by `channel_scope_briefing` — CANAL-ESCOPO.md Shape
/// 3's T1 layer. v3: the injected channel prefix now carries `seq=N`, so
/// every pane briefed under v2 must be re-briefed — it was told a shape
/// the runtime no longer emits, and `--after <seq>`/`--reply-to <seq>`
/// were unusable without a visible seq. v4: an unresolved `@nick` is now
/// a loud error rather than a silent broadcast, and the `name` rungs
/// start at the workspace label (ceo-bora#30/#31) — v3 panes were told
/// the OPPOSITE of what the runtime now does ("never resend because of
/// that; the message already went out to everyone"), which is the one
/// kind of stale briefing that makes an agent act wrongly rather than
/// merely miss a feature. v5: channel fan-out delivery defaults to
/// IMMEDIATE injection, even when the recipient is mid-turn (steering
...
impl App {
    ...
    /// `removed: false` means there was nothing to drop from the roster,
    /// either because the pane never joined or because it lives in the
    /// channel's workspace and is a member by construction.
    ...
    /// — exempt from the agent-prompt rate limit and carries no
    /// `[from ...]` prefix — and the caller's `when_idle` mode: the
    /// `channel.send` fan-out passes the message's own mode so the
