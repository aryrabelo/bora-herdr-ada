...
impl App {
    ...
    /// Completes `worktree.open` after `reads.rs` resolves the source and
    /// entry off the main thread. `target_matches` re-validates the checkout
    /// against live git-space metadata before mutating anything, because the
    /// filesystem discovery that produced `entry` ran asynchronously and the
    /// checkout may have changed underneath it by the time this runs.
