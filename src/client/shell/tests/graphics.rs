...
fn assert_graphics_cover(state: &mut ClientShellState, covered: Rect, cols: u16, rows: u16) {
    ...
    assert!(
        !is_placed(&frame.graphics.into_inline_bytes(), inside),
...
fn selection_copy_cursor_and_search_hide_only_the_highlighted_images() {
    ...
    assert!(is_placed(&frame.graphics.into_inline_bytes(), points[3]));
    ...
    assert!(!String::from_utf8_lossy(&frame.graphics.into_inline_bytes()).contains("a=t"));
}
...
fn mobile_switcher_still_hides_the_entire_underlying_surface() {
    ...
    assert!(is_placed(&frame.graphics.into_inline_bytes(), point));
    ...
    assert!(String::from_utf8_lossy(&frame.graphics.into_inline_bytes()).contains("a=d,d=i"));
    state.mode = ClientShellMode::Terminal;
    let frame = state.compose(40, 24).unwrap();
    assert!(is_placed(
        &frame.graphics.clone().into_inline_bytes(),
...
fn popup_terminal_keeps_own_graphics_and_hides_only_background_overlap() {
    ...
    assert!(is_placed(&frame.graphics.into_inline_bytes(), inside));
