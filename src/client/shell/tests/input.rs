...
fn client_shell_graphics_follow_final_shell_origin_and_local_overlay_visibility() {
    ...
    let visible = visible.graphics.into_inline_bytes();
    ...
    assert!(String::from_utf8_lossy(&uncovered.graphics.into_inline_bytes()).contains("a=p"));

    state.overlay = None;
    let restored = state.compose(106, 20).expect("restored graphics frame");
    let restored = restored.graphics.into_inline_bytes();
