...
impl ClientShellState {
    ...
    pub(super) fn move_navigate_workspace(&mut self, delta: isize) {
        ...
        for endpoint in &self.endpoints {
            ...
            // PR #40 regression guard: for the active endpoint the walk has to
            // mirror the order the sidebar actually renders (view-mode aware),
            // or prev/next steps into rows a collapsed folder is hiding.
            // Remote endpoints render via `workspace_entries` in the expanded
            // sidebar regardless of view mode, so upstream's per-endpoint walk
            ...
            let entries = if self.sidebar_collapsed && !mobile && surface_available {
                snapshot
                    ...
                    .collect()
            } else if endpoint.endpoint_id == self.active_endpoint_id {
