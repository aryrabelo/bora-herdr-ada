...
fn list(args: &[String]) -> std::io::Result<i32> {
    let json = match args {
        ...
        _ => {
            eprintln!("usage: bora machine list [--json]");
            return Ok(2);
        }
    };
...
fn parse_add_args(args: &[String]) -> Result<AddArgs, String> {
    ...
    let target = target.ok_or_else(|| {
        "usage: bora machine add <ssh-target> --label <label> [--remote-session <name>]".to_owned()
    })?;
...
fn rename(args: &[String]) -> std::io::Result<i32> {
    ...
    let [raw_id, flag, label] = args.as_slice() else {
        eprintln!("usage: bora machine rename <profile-id> --label <label>");
        return Ok(2);
    };
    if flag != "--label" {
        eprintln!("usage: bora machine rename <profile-id> --label <label>");
...
fn remove(args: &[String]) -> std::io::Result<i32> {
    let Some(id) = one_profile_id(args, "usage: bora machine remove <profile-id>")? else {
        return Ok(2);
    };
    let mut catalog = load_catalog()?;
...
fn set_enabled(args: &[String], enabled: bool) -> std::io::Result<i32> {
    ...
    let usage = format!("usage: bora machine {action} <profile-id>");
