...
//! explicit-membership roster and per-member read-cursor registry that live
//! beside it.
//!
...
//! ids. Each member's high-water read cursor lives at
//! `state_dir()/channels/<name>.cursors.json` (CANAL-NAO-LIDO's unread
//! primitive) — the seq a `channel tail` / `channel history` read has
//! advanced that member through.
