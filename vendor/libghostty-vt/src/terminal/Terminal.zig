...
pub fn resize(
    ...
    //
    // Reflow requires both wraparound (the program expects soft-wrapped
    // lines) and the host opting in to reflow-on-resize.
...
test "Terminal: resize with reflow_on_resize disabled survives full reset" {
    const alloc = testing.allocator;
    const io_impl = testing.io;
    var t = try init(io_impl, alloc, .{ .cols = 4, .rows = 2 });
    defer t.deinit(alloc);
