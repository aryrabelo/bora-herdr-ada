...
fn dead_server_cli_reports_one_session_aware_json_line() {
    ...
    assert_server_not_running(missing, &named_socket, "bora session attach foo");

    let stale_socket = runtime_dir.join("stale.sock");
    drop(UnixListener::bind(&stale_socket).unwrap());
    let stale = Command::new(env!("CARGO_BIN_EXE_bora"))
...
fn status_commands_report_client_and_server_versions() {
    ...
    assert!(
        full_stdout.contains("  protocol: 25"),
        "stdout: {full_stdout}"
    );
    ...
    assert!(
        server_stdout.contains("private_protocol: 25"),
        "stdout: {server_stdout}"
    );
    ...
    assert!(
        client_stdout.contains("protocol: 25"),
        "stdout: {client_stdout}"
    );
    assert!(
    ...
    assert_eq!(server_json["protocol"], 25);
