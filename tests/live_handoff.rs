...
fn spawn_server_with_env(
    ...
    fs::write(config_home.join("bora/config.toml"), "onboarding = false\n").unwrap();
...
fn live_handoff_preserves_named_session_socket_paths() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let session_dir = config_home.join("bora-dev/sessions/work");
    ...
    assert!(
        !config_home.join("bora-dev/herdr.sock").exists(),
...
fn live_handoff_preserves_pane_process_io() {
    ...
    // macOSunix-socket buffers are small (8 KiB default). Frames the test
    // client never reads can fill the buffer and block the server's writer
    // task, which would then never flush the handoff ServerShutdown before
    // the old process is killed below. Drain first so the shutdown message
    // has a clear path. (Linux CI defaults to ~212 KiB, which is why this
    // only flakes on macOS.)
    support::drain_messages(&mut client_stream);
    assert_ok(request(
        &api_socket,
        serde_json::json!({"id":"test:handoff","method":"server.live_handoff","params":{}}),
    ));
...
fn live_handoff_preserves_http_servers_across_multiple_sessions() {
    ...
    let runtime_dir = base.join("runtime");
    let sessions = [
        (None, config_home.join("bora-dev/herdr.sock")),
        (
            Some("work"),
            config_home.join("bora-dev/sessions/work/herdr.sock"),
        ),
    ];
