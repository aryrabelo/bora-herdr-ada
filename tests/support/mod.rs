...
pub fn drain_messages(stream: &mut UnixStream) {
    // Best-effort (Darwin quirk) and long, for the same mid-frame reason
    // as in wait_for_message_variants.
...
pub fn wait_for_message_variants(
    ...
    // Best-effort (Darwin SO_RCVTIMEO/EINVAL on torn-down connections) and
    // deliberately long: a short timeout can fire mid-frame — after the
    // length prefix but before the payload — dropping the partial bytes and
    // permanently desyncing the stream. The outer deadline still bounds the
    // total wait.
    let _ = stream.set_read_timeout(Some(Duration::from_secs(5)));
    let deadline = Instant::now() + timeout;
    while Instant::now() < deadline {
...
fn is_test_herdr_binary(path: &Path) -> bool {
    ...
    TEST_BINARY
        .get_or_init(|| fs::canonicalize(env!("CARGO_BIN_EXE_bora")).ok())
        .as_deref()
...
mod tests {
    ...
    fn test_binary_matcher_accepts_cargo_test_binary() {
        let binary = std::fs::canonicalize(env!("CARGO_BIN_EXE_bora"))
    ...
    fn test_binary_matcher_rejects_other_binaries() {
        ...
        let sibling_build = Path::new(env!("CARGO_BIN_EXE_bora"))
            .parent()
            .unwrap()
