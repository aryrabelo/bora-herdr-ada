...
右侧面板显示活动工作区所属仓库的上下文。可以用配置的键位(默认:`prefix+shift+b`)切换,或使用终端右侧边缘的折叠状态开关。

该面板有四个标签页:

- **Changes** — 未暂存、已暂存和已提交的文件更改,带状态指示器和每个文件的差异计数。点击文件可在 gitui 中打开(若未安装 gitui,则使用 `git diff`)。
- **Checks** — 活动工作区分支的 CI 检查状态,通过 `gh pr view` 按需获取。
- **Issues** — 分配给当前用户或由当前用户创建的未关闭 issue,按需获取。点击 issue 可在浏览器中打开或复制其 URL。
- **PRs** — 当前用户在该仓库中打开的 pull request,在打开标签页时以及后台定期刷新。每一行显示 PR 编号、标题、草稿标记,以及可合并性指示器(`✓` 绿色 = MERGEABLE,`✗` 红色 = CONFLICTING)。点击 PR 可打开其上下文菜单(Open in worktree / Open in browser / Copy URL)。
