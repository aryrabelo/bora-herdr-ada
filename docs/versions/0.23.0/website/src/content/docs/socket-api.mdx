...
| GitHub | `github.pulls.list`, `github.issues.list` |
...
Herdr periodically fetches the current user's open pull requests per repo in the background when `[github] enabled = true` (default). Issues are fetched on demand. `github.pulls.list` and `github.issues.list` expose the cached data through the socket API.

List open PRs across all repos:

```json
{"id":"req_1","method":"github.pulls.list","params":{}}
```

Filter by a specific repo identity:

```json
{"id":"req_1","method":"github.pulls.list","params":{"repo_identity":"github.com/owner/repo"}}
```

Each response returns a `repos` array. Each repo entry includes `repo_identity`, a `prs` (or `issues`) array with `number`, `title`, `url`, and for PRs `head_ref_name` and `is_draft`, plus an optional `error` field when the `gh` CLI failed for that repo. When no data is cached, `repos` is an empty array.

Subscribe to `github.prs_refreshed` and `github.issues_refreshed` to be notified when a background refresh completes. Each event includes the `repo_identity` that was refreshed.

`github.pr_opened` fires once when bora successfully opens a pull request for a worktree branch. Unlike the poll-based refresh events above, it is pushed immediately at creation time rather than discovered on the next periodic refresh. The event carries the `branch`, PR `url`, `repo_identity`, and affected `workspace_ids`. `workspace_ids` only includes workspaces belonging to that same repository; branch name alone is not enough to identify a workspace when several repositories are open. It only covers PRs opened by bora itself, not PRs opened by running `gh` directly in an agent pane.
