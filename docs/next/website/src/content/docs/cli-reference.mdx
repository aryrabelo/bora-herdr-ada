...
herdr agent prompt <target> <text> [--announce] [--wait] [--until STATUS]... [--timeout MS]
...
`agent prompt` always emits an `agent.prompted` event (subscribable via `events`/`events.wait`) carrying the sender pane, target pane and workspace, and the submitted text truncated to 4KB. The sender pane comes from `HERDR_PANE_ID` when the CLI runs inside a Herdr pane; it is otherwise absent. `--announce` additionally prepends a `[from <pane_id>]` header line to the text actually written to the target pane, so the receiving agent — which has no channel but its own terminal input — can see who is prompting it. `--announce` requires `HERDR_PANE_ID` to be set and is off by default, since it mutates the submitted payload and would corrupt callers piping exact commands or code.
