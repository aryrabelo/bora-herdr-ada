---
title: Socket API
description: Control a running Bora server from scripts, tools, and coding agents.
---

Bora exposes a local socket API for scripts and agents that need to inspect or control a running session.
...
| Layer | Use it for |
| --- | --- |
| Agent skill | Teaching a coding agent how to use Bora from inside a pane. |
...
The installed CLI can print the socket protocol schema bundled with that Bora
binary:
...
Plain `bora api schema` prints a short summary. `--json` prints the full JSON
...
```bash
bora workspace create --cwd ~/project --label api
```

Create a tab:

```bash
bora tab create --label logs
```

Split a pane and run a command:

```bash
bora pane split w1:p1 --direction right
bora pane run w1:p2 "npm test"
```

Inspect and rearrange panes:

```bash
bora pane layout --current
bora pane neighbor --direction right --current
bora pane resize --direction right --amount 0.1 --current
bora pane swap --direction right --current
bora pane zoom --on --current
bora pane split w1:p1 --direction right --ratio 0.333
```

Wait for an agent:

```bash
bora agent wait w1:p1 --until done
```

Read pane output:

```bash
bora pane read w1:p2 --source recent --lines 50
```
...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.report_agent`, `pane.set_status`, `pane.report_agent_session`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Popup | `popup.close` |
...
From the CLI, `bora api snapshot` prints the live `session.snapshot` response
...
`pane.send_keys` and `pane.send_input.keys` accept Bora key-combo strings:
...
Bora returns that pane. When it is omitted, Bora returns the active focused
pane.

`pane.input.set` sets `right_click` to `bora` or `pane` for one pane. `bora`
is the default. `pane` forwards unmodified right-click hold and drag gestures
when the application requests terminal mouse reporting; otherwise Bora falls
back to its pane menu. Right-clicking the pane frame always opens Bora's menu.
...
`bgra`; BGRA is normalized once to owned RGBA. Bora advances the host cache
...
Bora replies with `ok`, send one JSON header and then exactly `data_length` raw
...
inline. Bora replies with a `pane_graphics_frame_ack` only after the terminal
...
placed during a redraw, Bora uploads the image without displaying it and replays its
placement when the pane becomes visible again.
`file_frame_damage: true` means Bora accepts optional damage metadata for
...
provided, Bora creates the replacement tab first and then closes the old tab.
...
Process-launching methods accept an `env` object. Bora applies those key/value
pairs to the newly launched process only. Bora also injects `HERDR_SOCKET_PATH`,
`HERDR_ENV=1`, `HERDR_WORKSPACE_ID`, `HERDR_TAB_ID`, and `HERDR_PANE_ID` into
managed pane processes. Bora-managed variables are authoritative when they
...
`pane.moved`; Bora does not emit fake pane close/create events for the moved
terminal process.
...
```bash
bora notification show "build failed" --body "api workspace" --position top-left --sound request
```
...
`title` is required and must contain visible text after control characters and repeated whitespace are removed. `body` is optional. Bora collapses newlines, tabs, carriage returns, and repeated whitespace into spaces, then trims notification text to 80 characters for `title` and 240 characters for `body`. An empty sanitized `title` returns `invalid_params`. `position` is optional and applies only when `ui.toast.delivery = "herdr"`; desktop positions are relative to the full Bora frame, and omitted positions use `ui.toast.herdr.position`. Terminal, system, and off delivery ignore `position`. `sound` is optional and can be `none`, `done`, or `request`; it defaults to `none` and plays only when the notification is shown.
...
Possible reasons are `shown`, `disabled`, `rate_limited`, `no_foreground_client`, and `busy`. `disabled` means `ui.toast.delivery = "off"`. `busy` means an existing in-app toast was not replaced. Terminal and system delivery are best-effort through the current foreground attached Bora client.

Set or clear the foreground client's outer terminal window title:

```json
{"id":"req_title","method":"client.window_title.set","params":{"title":"bora api"}}
...
Worktree methods manage Git checkouts as Bora workspaces. `worktree.create` creates a checkout and returns the new `workspace`, `tab`, `root_pane`, and `worktree` records. If the requested branch already exists locally, it checks out that branch; otherwise it creates the branch from the requested base or `HEAD`. `worktree.open` opens an existing checkout or returns the already-open workspace. `worktree.remove` runs `git worktree remove` against a linked child workspace and never deletes the branch.
...
Use at most one of `workspace_id` or `cwd` for `worktree.list`, `worktree.create`, and `worktree.open`; omit both to use the active workspace. Use exactly one of `path` or `branch` for `worktree.open`. Raw socket `cwd` and `path` values must be absolute; the CLI expands relative `--cwd` and `--path` values before sending requests. Workspace responses include optional `worktree` provenance when a workspace belongs to a Bora worktree group. Worktree commands can emit `workspace.updated` when an existing workspace gains or changes worktree provenance.

Worktree commands also emit lifecycle events. `worktree.create` emits `workspace.created`, `tab.created`, `pane.created`, and `worktree.created`. `worktree.open` emits `worktree.opened`, and it also emits workspace/tab/pane creation events when it opens a new Bora workspace. `worktree.remove` emits `worktree.removed`; if the linked workspace is still open, it also emits `workspace.closed`.

`workspace.close` closes only the named workspace unless its params include `"close_group": true`, which also closes the linked-worktree workspaces sharing its checkout. A group close emits one `workspace.closed` event for each workspace it closes.
...
`source` identifies the owner. Plugins use `plugin:<HERDR_PLUGIN_ID>`; Bora
...
Installed and linked plugins persist across restarts. Bora writes a
`plugins.json` registry file alongside `session.json` on `plugin.link`,
`plugin.unlink`, `plugin.enable`, and `plugin.disable`. The
`bora plugin install` and `bora plugin link` CLIs also write the same registry
when Bora is not running, then startup loads it automatically. On startup,
Bora re-reads each manifest from its original path; if the file is missing or
unparseable, the entry is kept with a `warnings` field so `plugin.list` surfaces
it.

Bora validates event hook `on` values against known event names at link time.
...
field is missing, invalid, or newer than the running Bora binary.
...
command, and returns the Bora-built invocation context plus the started command
...
Bora injects `HERDR_SOCKET_PATH`, `HERDR_BIN_PATH`, `HERDR_ENV=1`,
...
Event hooks run for enabled installed plugins when Bora emits a matching event
name such as `worktree.created`.

There is no Bora-managed plugin storage API in v1. `HERDR_PLUGIN_CONFIG_DIR`
...
behave like normal Bora panes, and `plugin.pane.focus` and `plugin.pane.close`
continue to operate on those panes.

## Socket transport

Bora uses newline-delimited JSON over a local socket. On Unix, that socket is a
...
The default socket lives under your Bora config directory.

Named sessions have separate sockets:

```text
~/.config/bora/herdr.sock
~/.config/bora/sessions/<name>/herdr.sock
```
...
`pane.set_status` pins a pane's status by hand, overriding detection wherever
status is read out: `PaneInfo`, `AgentInfo`, the tab and workspace rollups, and
`pane.agent_status_changed` payloads. `status` accepts `working`, `blocked`,
`idle`, or `done`; `null` clears the override. `unknown` is rejected with
`invalid_params`. Detection keeps running underneath a pin, so clearing one
immediately reveals the live state. When several panes of one workspace or tab
are pinned, the most recently set pin wins the rollup. Pins are runtime-only and
do not survive a server restart.

```json
{
  "id": "req_3",
  "method": "pane.set_status",
  "params": {
    "pane_id": "w1:p1",
    "status": "blocked"
  }
...
`pane.get`, `pane.list`, `agent.get`, and `agent.list` expose a read-only `agent_session` object when Bora has a stored native session reference:
...
`pane.get`, `pane.list`, `agent.get`, and `agent.list` also expose `foreground_cwd` when Bora can resolve the cwd of the process currently controlling the pane PTY. The existing `cwd` field remains the pane/workspace cwd used for labels, follow-cwd behavior, and restored session state.
...
Use `pane.report_metadata` when a user hook wants to customize presentation without taking over lifecycle state from a Bora integration.
...
Presentation text is normalized before storage. Bora trims surrounding whitespace, removes control characters, and caps `title`, `display_agent`, each state label, and token values at 80 characters. Empty normalized token values clear that key.
...
All entries must be valid: if any referenced pane does not exist, Bora rejects
...
`pane.moved`, `pane.exited`, `pane.agent_detected`, `pane.result_reported`,
...
`workspace_id`, and the current `scroll` metrics whenever Bora observes a
...
Channel event subscriptions include `channel.message`, emitted right after a message is durably appended to a `#channel` transcript and before it is fanned out to the channel's members. It carries the normalized `channel` name without the leading `#`, the monotonic per-channel `seq`, `from_name`, and `text`; `from_pane` is omitted for unattributed senders, and `to_pane` carries the resolved pane id when the message was addressed to one member instead of broadcast. The subscription takes no parameters — a client interested in one room filters on `channel` rather than subscribing per room.
...
```bash
bora pane read w1:p1 --source visible --lines 80
bora pane read w1:p1 --source recent --lines 120
bora pane read w1:p1 --source recent-unwrapped --lines 120
bora pane read w1:p1 --source detection
```
...
```bash
bora agent wait w1:p1 --until done
bora agent wait w1:p1 --until blocked
```
...
```json
{
  ...
  "result": {
    ...
    "manifests": [
      {
        "agent": "cursor",
        "source": "/home/me/.config/bora/agent-detection/cursor.toml",
...
The response contains the same explain object printed by `bora agent explain --json`, including the final state, manifest source and version, matched rule, evaluated rule evidence, skip-state reason, idle fallback reason, and `screen_detection_skip_reason` when a full lifecycle hook authority makes screen rules non-authoritative.

Clients need a running server that supports `agent.explain`; after upgrading Bora, restart or live-handoff the server before relying on this method.
...
The client-rendered Bora UI uses a stable endpoint generation for local and SSH
...
including direct terminal attach and live handoff. Check `ping` or `bora status`
