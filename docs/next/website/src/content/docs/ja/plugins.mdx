---
title: プラグイン
description: マニフェストのアクション、イベントフック、ペインを備えたローカル Bora プラグインを作成します。
---

Bora プラグインは、共有可能で実行可能なワークフローパッケージです。プラグインは
Bash スクリプト、JavaScript アプリ、Lua スクリプト、Rust バイナリ、その他マシンで
実行できる任意の argv コマンドにできます。Bora はホスト側の面を担います:
インストール、マニフェスト検証、キーバインド、ターミナルペイン、イベント、
呼び出しコンテキスト、ソケットアクセスです。プラグインは自分の実装言語、依存関係、
ファイル、永続状態を担います。

プラグインは Bora を軽量に保つために存在します。コアはターミナルワークスペース、
ペイン、エージェント、安定した CLI/ソケット API に集中し続けます。プラグインは
その既存の拡張面を、あらゆるワークフローを Bora 本体に追加することなく、
誰もが作成・インストール・共有できる再利用可能なワークフローに変えます。

プラグインは SDK インテグレーションではありません。`herdr-plugin.toml`
マニフェストと、Bora が起動できるコマンドを持つディレクトリです。Bora は
マニフェストを検証し、ランタイムコンテキストを注入し、宣言されたコマンドを起動し、
ログを記録します。コマンドはさらに作業が必要なときに、CLI またはソケット経由で
Bora を呼び出します。

独立したプラグイン SDK や制限付きコマンドセットはありません。Bora CLI 全体が
プラグイン API です: [CLI リファレンス](/ja/docs/cli-reference/)のすべてのコマンドを
プラグインから使えます。自分で `bora ...` として実行できるものは、プラグインも
実行できます。ほとんどのプラグインは、実行中の Bora バイナリを指す
`HERDR_BIN_PATH` を通じて Bora を呼び出すべきです。これにより、Unix ソケットと
...
実行され、Bora CLI 全体を呼び出せます — エディタ、シェル、コーディングエージェントに
...
スクリプトやバイナリです。`bora plugin install` は対話的なターミナルでソースと
実行されるコマンドのプレビューを表示するので、確定する前に確認できます。すでに
信頼しているソースには `--yes` を使い、特定のリビジョンが欲しいときは `--ref` で
固定してください。

Bora はマニフェストを検証し、各プラグインの設定と状態を専用ディレクトリに
保ちますが、プラグインの動作をレビューしたりサンドボックス化したりはしません。
サードパーティのプラグインは Bora ではなく作者のものです。検証と実行の判断は
あなた自身に委ねられます。

## マニフェスト

マニフェストは Bora とプラグインの間の契約です。パッケージのメタデータ、対応
プラットフォーム、任意のビルドコマンド、Bora が実行できるエントリーポイントを
宣言します。
...
マニフェストフィールドをサポートする最も古い Bora バージョンを設定してください。
プラグインの最低バージョンが現在のバイナリより新しい場合、Bora はリンクや
...
一意な名前が必要なとき、Bora はアクション id を `plugin.id.action` の形に修飾します。
...
`command` の値は argv 配列です。Bora はこれをシェル経由で実行しないため、
...
コマンドの中からは `HERDR_BIN_PATH` で Bora を呼び出します:
...
ビルドコマンドは GitHub からの `plugin install` 中、確認の後、Bora がプラグインを
...
Bora のソケット環境変数は受け取りません。プラグインの作者は `cargo`、`npm`、
`bun`、`lua` といった必要なシステムツールをドキュメントに書いてください。Bora は
ビルドの失敗を報告しますが、足りないツールチェーンをインストールすることはありません。

## スタートアップフック

`[[startup]]` コマンドは、Bora がセッションを復元し、API ソケットの準備が整った後に、
有効なプラグインごとに一度実行されます。ライブハンドオフで新しいサーバーが引き継いだ
ときにも再実行されますが、クライアントの接続、設定の再読み込み、プラグインの link や
enable では実行されません。Bora はこれらを非同期で開始し、完了を通常のプラグイン
コマンドログに記録します。スタートアップの失敗でサーバーが停止することはありません。

スタートアップフックは一度限りの初期化コマンドであり、監視されるデーモンではありません。
フックはプラグインが所有する状態を復元し、必要な Bora API を呼び出して終了してください。
...
Bora は `HERDR_SOCKET_PATH`、`HERDR_BIN_PATH`、`HERDR_ENV=1`、
...
ランタイム状態は `HERDR_PLUGIN_STATE_DIR` の下に置いてください。Bora はこれらの
...
Node、PowerShell、Bash など別のランタイムから移植性を保って Bora を呼び出す必要が
...
`placement = "tab"` のペインは、マニフェストのペインの `title` をタブ名として
使う新しいタブを開くため、プラグインは開いた後にタブ名を変更する必要は
ありません。
...
split、tab、zoomed、overlay のプラグインペインは、開いた後は通常の Bora ペインです。プラグインはソケットや CLI を
通じて `pane.move`、`pane.swap`、`pane.resize`、`pane.zoom` といった標準のペイン
API を呼び出せます。ペインがタブやワークスペースをまたいで移動しても、Bora は
プラグインペインの所有権を元のペインに結び付けたまま維持します。
ポップアップは Bora ペインではなく、セッションに 1 つだけ存在できるリソースです。
...
Settings、コピーモード、または別の Bora モーダルが開いている間にポップアップを開くと `ui_busy` が返り、起動後の `plugin.pane.open` は `ok` を返します。

Windows では、ビルドコマンド、アクションコマンド、イベントコマンドは、素の
コマンドが `PATH` にあれば `npm.cmd`、`bun.cmd`、`pnpm.cmd` のような一般的な
`PATHEXT` shim を解決します。ペインコマンドは Bora の通常の Windows ペイン
...
v1 には Bora が管理するプラグインストレージ API はありません。永続状態が必要な
...
`bora plugin install owner/repo[/subdir]` を共有してください。
