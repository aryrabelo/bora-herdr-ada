...
`github.pr_opened` は、bora が worktree のブランチに対してプルリクエストを開くことに成功した際に一度だけ発行されます。上記のポーリング型のリフレッシュイベントとは異なり、次回の定期リフレッシュを待たず、作成時点で即座にプッシュされます。イベントには `branch`、PR の `url`、影響を受けた `workspace_ids` が含まれます。これは bora 自身が開いた PR のみを対象としており、エージェントのペイン内で直接 `gh` を実行して開いた PR は対象になりません。
