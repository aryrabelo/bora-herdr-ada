...
template: splash
hero:
  tagline: "Bora のインストール、学習、設定。あなたに合ったパスから始めましょう — マルチプレクサの経験は不要です。"
  image:
...
<CardGrid>
  <Card title="ターミナルマルチプレクサは初めて?">
    ショートカットを覚えなくても始められます。Bora はマウスファーストです。ペインをクリックし、境界をドラッグし、右クリックメニューから分割や切り替えができます。
...
このガイドはエージェントに Bora のコンセプト、セットアップ、設定、よくある問題の解決方法を教えるので、エージェントの回答が即興ではなく正確なままになります。
...
<CardGrid>
  ...
  <Card title="設定">
    ...
    [Bora を設定する →](/ja/docs/configuration/)
  </Card>
  <Card title="API">
    CLI とローカルソケット API を通じて、スクリプト・ツール・エージェントから Bora を制御します。
