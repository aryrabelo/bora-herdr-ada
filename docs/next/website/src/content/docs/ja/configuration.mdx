...
右パネルは、アクティブなワークスペースのリポジトリに関するコンテキストを表示します。設定したキーバインド(デフォルト: `prefix+shift+b`)、またはターミナル右端の折りたたみ状態トグルで切り替えます。

パネルには 4 つのタブがあります:

- **Changes** — 未ステージ、ステージ済み、コミット済みのファイル変更を、ステータスインジケーターとファイルごとの差分件数とともに表示します。ファイルをクリックすると gitui で開きます(gitui が未インストールの場合は `git diff`)。
- **Checks** — アクティブなワークスペースのブランチの CI チェック状態を、`gh pr view` でオンデマンド取得して表示します。
- **Issues** — 現在のユーザーに割り当てられた、または作成された未クローズの issue をオンデマンドで取得して表示します。issue をクリックするとブラウザで開くか、URL をコピーできます。
- **PRs** — そのリポジトリで現在のユーザーが開いているプルリクエストを表示し、タブを開いたときとバックグラウンドで定期的に更新します。各行には PR 番号、タイトル、ドラフトマーク、マージ可否インジケーター(`✓` 緑 = MERGEABLE、`✗` 赤 = CONFLICTING)が表示されます。PR をクリックするとコンテキストメニュー(Open in worktree / Open in browser / Copy URL)を開きます。
