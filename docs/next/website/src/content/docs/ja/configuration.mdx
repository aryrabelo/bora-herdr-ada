...
デフォルトでは、ペインの幅が変わると Herdr は既存の内容を折り返し直すため、ソフトラップされた論理行は新しい幅で再分割されます。この折り返しは、プログラムが以前の幅に合わせて空白で埋めた出力も再分割してしまい、1 行のパディング済み行が「内容の行」と「末尾が空白だけの残り行」に分かれます。`reflow_on_resize = false` にすると、折り返す代わりに新しい幅で行を切り詰め、再描画は実行中のプログラムに任せます（tmux と同じ挙動です）。この設定は変更後に作成されたペインに適用されます。
