...
默认情况下，窗格宽度变化时 Herdr 会重新折行现有内容，因此软换行的逻辑行会按新宽度重新拆分。这种重新折行同样会拆分程序按旧宽度用空格补齐的输出，使一行补齐过的内容变成一行正文加一行只有尾部空格的残余行。设为 `reflow_on_resize = false` 时，Herdr 会按新宽度截断各行，并把重绘交给正在运行的程序处理（与 tmux 的行为一致）。该设置对修改之后创建的窗格生效。
