...
`workspace close` 只关闭 Herdr 状态。关闭父工作区只会关闭该工作区；传入 `--group` 才会同时关闭与其共享检出的关联 worktree 工作区。`worktree remove` 是显式的检出删除路径;它运行 `git worktree remove`,从不删除分支,并在 Git 拒绝脏检出时要求 `--force`。
