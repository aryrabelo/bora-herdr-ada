...
`github.pr_opened` 会在 bora 成功为某个 worktree 分支打开 pull request 时触发一次。与上面基于轮询的刷新事件不同,它是在创建时立即推送的,而不是等到下一次定期刷新才被发现。该事件携带 `branch`、PR 的 `url` 以及受影响的 `workspace_ids`。它只覆盖 bora 自己打开的 PR;在智能体窗格中直接运行 `gh` 打开的 PR 不会触发它。
