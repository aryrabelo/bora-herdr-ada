...
应用和插件应通过窗格的终端输出发送标准 Kitty 图形协议。Bora 在兼容的外层终端中管理图像的位置、裁剪和可见性。

外部 socket 窗格图像叠加 API 已移除:`pane.graphics.info`、`pane.graphics.set`、`pane.graphics.clear` 和 `pane.graphics.stream` 不再可用,也没有替代的 socket 图像 API。与旧服务器的内部传输协议兼容性不会恢复这些公开方法。
