---
title: Configuration
description: Configure Bora keybindings, themes, sidebar behavior, notifications, and advanced options.
---

Bora works without a config file. Add one when you want custom keys, themes, sidebar layouts, notifications, or advanced behavior.
...
Bora reads config from:

```text
Linux and macOS: ~/.config/bora/config.toml
Windows:          %APPDATA%\bora\config.toml
```

Run `bora --help` to see the resolved config path for your system.
...
```bash
bora --default-config > ~/.config/bora/config.toml
```

If a config value is invalid, Bora falls back to a safe default and shows a startup warning.

Bora shows first-run setup when `onboarding` is missing or true. Continuing from onboarding writes `onboarding = false` and opens settings on the integrations tab. Set `onboarding = false` to skip that flow after setup.
...
You can also open the global menu in Bora and choose `reload config`.
...
Set the executable Bora uses for newly created interactive panes:
...
By default Bora rewraps the existing pane contents when the pane's width changes, so a soft-wrapped logical line is re-split at the new width. That rewrap also re-splits output a program padded to the old width, which turns one padded row into a content row plus a trailing-space remainder row. Set `reflow_on_resize = false` to truncate rows at the new width instead and leave repainting to the running program, which is what tmux does. The setting applies to panes created after the change.

## Worktrees

Set the root directory Bora uses for Git worktree checkouts created from the sidebar:
...
Bora creates checkouts under `<directory>/<repo>/<branch-slug>`. For sibling-style checkouts, set this to a directory such as `~/Projects/bora-worktrees`. Relative values are resolved to an absolute path when the app applies the config.

Worktree actions are available from Git workspace rows. `New worktree` creates a checkout. It checks out an existing local branch when the entered branch exists; otherwise, it creates the branch. It then opens the checkout as a new Bora workspace and groups it under the source workspace. `Open worktree...` lists existing Git worktree checkouts for that repo. Choosing an already-open checkout focuses it, while choosing a closed checkout opens it in the same group.

Each repo header row in the sidebar shows a `+` (create worktree) button. Clicking it opens the **Create worktree** modal, a single branch-name entry: it checks out the branch if it already exists locally, otherwise it creates the branch. The `new_worktree` keybind and the Git workspace context-menu `New worktree` entry open the same modal.

Grouped worktrees still behave like normal Bora workspaces: they can be focused, renamed, closed, and contain their own tabs and panes. The parent row is the original workspace. Closing the parent row closes the whole Bora group, but it does not delete checkout folders or branches.

To delete a worktree checkout, use `Delete worktree checkout...` on a grouped child workspace. Bora runs `git worktree remove`, first asking Git to remove safely. If Git refuses because the checkout has modified or untracked files, Bora asks again before running the forced remove. Branches are not deleted.
...
When enabled, `bora --remote` writes a private temporary SSH config that includes your user and system SSH configs first, then adds fallback `ServerAliveInterval` and `ServerAliveCountMax` values. Your own keepalive settings win. Linux and macOS clients also use a private per-attach OpenSSH control socket to reuse the first authenticated connection; Windows OpenSSH does not. Set `manage_ssh_config = false` to run remote attach through plain `ssh` without Bora's generated config or control socket.

## Keybindings

For a guided introduction to the prefix and a vetted prefix-free setup, see [Keyboard](/docs/keyboard/).

Bora has a prefix mode similar to tmux. The default prefix is `ctrl+b`. Keybinding strings are explicit: `prefix+n` means press the configured prefix and then `n`; `ctrl+alt+n` is a direct terminal-mode shortcut.
...
The default keymap is prefix-first so Bora does not steal input from shells, editors, tmux, or terminal apps. Search `keys.` in the [Config reference](/docs/config-reference/) to see every action and default binding. The in-app help panel at `prefix+?` shows the active bindings.
...
If you have old custom keybindings and want the new defaults, run `bora config reset-keys`. Bora backs up `config.toml`, removes `[keys]` and `[[keys.command]]`, and uses built-in v2 defaults after restart or `bora server reload-config`.
...
Custom commands receive `HERDR_SOCKET_PATH`, `HERDR_BIN_PATH`, `HERDR_ACTIVE_WORKSPACE_ID`, `HERDR_ACTIVE_TAB_ID`, `HERDR_ACTIVE_PANE_ID`, and `HERDR_ACTIVE_PANE_CWD` when those values are available. Shell commands run from the focused pane's working directory when Bora can detect it.
...
Search `theme.name` in the [Config reference](/docs/config-reference/) for every built-in theme. Use `terminal` when you want Bora UI colors to follow your host terminal's ANSI palette.

To let Bora switch its own UI theme when the host terminal reports a light/dark appearance change, enable theme auto-switching:
...
`auto_switch` defaults to `false`, so existing theme configs keep manual behavior. If `light_name` or `dark_name` is omitted, Bora uses the matching built-in sibling for the configured `name` when one exists, such as `tokyo-night`/`tokyo-night-day` or `gruvbox`/`gruvbox-light`. Manual theme selection in Settings disables `auto_switch`.
...
The sidebar is the main Bora dashboard. Search `ui.` in the [Config reference](/docs/config-reference/) for sizing, collapsed mode, Agent panel ordering, mouse behavior, pane borders, and other presentation settings.
...
Bora emulates the terminals in its panes, so an `OSC 0`/`OSC 2` title written inside a pane stops at Bora. Bora writes its own title to the terminal it runs in, which is what window managers and terminal tab bars read:
...
The title renders on the Bora server, so `{hostname}` names the machine the panes run on, including when you attach with `bora --remote` or run `bora` over SSH. Set `window_title = ""` to leave the outer terminal title alone.
...
Use `bora workspace report-metadata` in the same way for custom Space tokens. Unreported custom tokens disappear.
...
Bora can notify you when a background agent finishes or needs input:
...
Choose `bora` for an in-app toast, `terminal` for an outer-terminal notification that also works over SSH, `system` for the local OS notification service, or `off` to disable popups. Bora suppresses popups for the active tab. Search `ui.toast` in the [Config reference](/docs/config-reference/) for positions, delay behavior, and clipboard feedback settings.
...
Sound notifications play through the local Bora client. Custom sounds must be mp3 files; Bora resolves relative paths from the config file's directory.
...
Applications and plugins integrate through the standard Kitty graphics protocol emitted in pane terminal output. Bora renders these images by default in compatible outer terminals. The external socket pane-overlay API has been removed; there is no replacement socket image API.
...
Bora automatically chooses eligible file transports, with no application changes or environment switches. On local Unix Ghostty clients, it can send image data through temporary files instead of encoding it into terminal output. A small file-consumption probe checks support first. Other terminals, SSH-launched clients, and nested multiplexers retain inline output for this last-hop optimization.

For eligible local clients, native RGBA images travel from server to client as paths to Bora-owned snapshots rather than pixel bytes. Unsupported clients and failed transfers fall back to inline delivery. Remote server paths are never forwarded to the local terminal. Bora keeps image placement, clipping, and visibility under its control.

On Linux, whole-file, uncompressed RGBA uploads can use copy-on-write snapshots before Bora reads their pixels. This requires filesystem support for cloning into Bora's private `/var/tmp` store. Unsupported filesystems, temporary-file uploads, shared memory, and other formats use the normal loader. Snapshots are limited to 16 MiB each and 64 MiB total; animation and unsupported clients read their pixels when needed. Bora never relies on a mutable producer path after accepting an image.

PNG uploads retain full decoding and validation. Bora does not defer corrupt-image validation to the outer terminal.
...
Changing this setting requires restarting the affected Bora server or reattaching the client. In remote sessions, the server's setting controls Kitty graphics parsing, while the local client's setting controls output to the outer terminal.

## Agent session restore

Bora resumes supported Agent conversations after a server restart by default:
...
Automatic restores start one agent at a time, 100 milliseconds apart by default, to reduce startup contention. The first eligible agent starts immediately. Set `startup_per_agent_delay_ms = 0` to disable spacing. Manual launches and explicit terminal attaches are unaffected. Restart the Bora server to apply changes.
...
On macOS and Windows, Bora can temporarily switch to an ASCII-capable input source while prefix commands and prefix-launched modes are active:
...
On macOS this switches to the current ASCII-capable keyboard layout; on Windows it switches the IME to English (ASCII) input. Bora restores the previous input source when returning to terminal input or entering a text field. This setting has no effect on other platforms.
...
| `HERDR_LOG` | Set log filtering, for example `HERDR_LOG=bora=debug`. |
...
```text
~/.config/bora/herdr.log
~/.config/bora/herdr-client.log
~/.config/bora/herdr-server.log
```
