...
Each repo header row in the sidebar shows a `+` (create worktree) button. Clicking it opens the **Create worktree** modal, the primary way to start a worktree. The modal has **GitHub / Branch / Name** tabs (switch by clicking a tab or pressing `Tab`):

- **GitHub** lists the repo's open pull requests and issues. Choosing a pull request creates or opens its worktree; choosing an issue runs the configured `[flow]` command. Issue rows are disabled with a hint when no `[flow]` command is set.
- **Branch** checks out an existing local branch into a new worktree.
- **Name** creates a fresh branch — the classic single-field behavior.

The `new_worktree` keybind and the Git workspace context-menu `New worktree` entry still open the same modal.
...
The right panel shows context for the active workspace's repository. Toggle it with the configured keybinding (default: `prefix+shift+b`) or the collapsed-state toggle at the right edge of the terminal.

The panel has four tabs:

- **Changes** — unstaged, staged, and committed file changes with status indicators and per-file diff counts. Click a file to open it in gitui (or `git diff` if gitui is not installed).
- **Checks** — CI check status for the active workspace's branch, fetched on demand via `gh pr view`.
- **Issues** — open issues assigned to or authored by the current user, fetched on demand. Click an issue to open it in the browser or copy its URL.
- **PRs** — the current user's open pull requests for the repo, refreshed on tab open and periodically in the background. Each row shows the PR number, title, draft marker, and a mergeable indicator (`✓` green = MERGEABLE, `✗` red = CONFLICTING). Click a PR to open its context menu (Open in worktree / Open in browser / Copy URL).
