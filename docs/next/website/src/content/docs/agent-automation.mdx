...
Channels group agents for broadcast. A channel is a workspace named `#…`; `channel create|list|members` manage and inspect them, `channel send` posts `[#name from <pane> <name>] <text>` to every member agent pane (with when-idle deferral and per-member `delivered|deferred|failed` results) and appends the message to a bounded per-channel JSONL transcript that `channel history` replays. Every message carries a monotonic per-channel `seq` that survives log rotation.
