...
Any pane that joins a channel, or receives its first message in one, gets a one-time briefing: a `[bora] channel protocol for #name (v1):` prompt teaching how to reply, address, and catch up in that channel. It's delivered through the same `agent prompt` path as any other injected text, so it's deferred while the pane is working and — because it carries no `from_pane` — exempt from the per-sender rate limit. Delivery is recorded per `(channel, pane, version)` in `channels/<name>.protocol.json`, so a server restart never re-sends it; bumping `CHANNEL_PROTOCOL_VERSION` does.
