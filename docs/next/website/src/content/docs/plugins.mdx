---
title: Plugins
description: Author local Bora plugins with manifest actions, event hooks, and panes.
---

Bora plugins are shareable, executable workflow packages. A plugin can be a
Bash script, JavaScript app, Lua script, Rust binary, or any other argv command
your machine can run. Bora owns the host surface: installation, manifest
...
Plugins exist so Bora can stay lean. The core stays focused on terminal
...
install, and share without adding every workflow to Bora itself.

A plugin is a directory with a `herdr-plugin.toml` manifest and commands Bora
can launch. Bora validates the manifest, injects runtime context, starts the
declared commands, and records logs. The commands call back into Bora through
the CLI or socket when they need to do more work.

There is no separate plugin SDK or restricted command set. The entire Bora CLI
is the plugin API. Every command in the
[CLI reference](/docs/cli-reference/) is available to a plugin, and a plugin can
run anything you can run yourself as `bora ...`. Most plugins should call Bora
through `HERDR_BIN_PATH`, which points at the running Bora binary. That keeps
...
commands run as your user, inherit your environment, and can call the full Bora
...
or binaries it runs. `bora plugin install` shows a preview of the source and
...
Bora validates the manifest and keeps each plugin's config and state in its own
directory, but it does not review or sandbox plugin code. Third-party plugins
come from their authors, not Bora; you are responsible for deciding whether to
run them.

## Manifest

The manifest is the contract between Bora and the plugin. It declares package
...
Set `min_herdr_version` to the oldest Bora version that supports the plugin
APIs, event names, and manifest fields your plugin uses. Bora refuses to link
...
dots. Each id type must be unique inside a plugin. Bora qualifies action ids
...
`command` values are argv arrays. Bora does not run them through a shell, so
...
Inside the command, call back into Bora with `HERDR_BIN_PATH`:

```js
const { spawnSync } = require("node:child_process");

const bora = process.env.HERDR_BIN_PATH ?? "bora";
const result = spawnSync(bora, ["workspace", "list"], {
  encoding: "utf8",
...
```bash
bora plugin install ogulcancelik/herdr-plugin-examples/agent-telegram-notify
bora plugin config-dir examples.agent-telegram-notify
bora plugin list
bora plugin action list --plugin examples.agent-telegram-notify
```

When you are authoring a local plugin, link the working directory instead:

```bash
bora plugin link /path/to/plugin
bora plugin config-dir example.layout
bora plugin action list --plugin example.layout
bora plugin action invoke example.layout.apply
bora plugin pane open --plugin example.layout --entrypoint board
bora plugin log list --plugin example.layout
```
...
Bora-managed plugin data and registers it. Use `--yes` for noninteractive
...
current user and available in every Bora session. Both `plugin install` and
`plugin link` can register plugins while no Bora server is running. Plugins
installed only in a named session on Bora 0.7.3 must be installed or linked
...
Bora does not maintain them as official plugins.

## Build commands

Build commands run during GitHub `plugin install` after confirmation and before
Bora registers the plugin. If a build command fails, install aborts and the
...
plugin context or Bora socket env. Plugin authors should document required
system tools such as `cargo`, `npm`, `bun`, or `lua`; Bora reports build
failures but does not install missing toolchains.

## Startup hooks

`[[startup]]` commands run once for each enabled plugin after Bora restores the
...
is linked or enabled. Bora starts them asynchronously and records their
...
daemons. A hook should restore plugin-owned state, call any required Bora APIs,
...
Runtime commands run with the plugin directory as their working directory. Bora
...
`HERDR_PLUGIN_STATE_DIR`. Bora creates those directories and seeds
...
Use `HERDR_BIN_PATH` when a plugin needs to call Bora portably from Node,
...
A `placement = "tab"` pane opens a new tab labeled with the manifest pane's
`title`, so a plugin does not need to rename the tab after opening it.
...
Split, tab, zoomed, and overlay plugin panes are normal Bora panes after they
open. Plugins can call standard pane APIs such as `pane.move`, `pane.swap`,
`pane.resize`, and `pane.zoom` through the socket or CLI; Bora keeps plugin
pane ownership attached to the underlying pane when it moves across tabs or
workspaces. A popup is a singleton session resource rather than a Bora pane:
...
Opening a popup returns `ui_busy` while Settings, Copy mode, or another Bora
...
command is on `PATH`. Pane commands use Bora's normal Windows pane launcher and
...
There is no Bora-managed plugin storage API in v1. Plugins that need durable
...
`bora plugin install owner/repo[/subdir]`.
