...
- The channels group header's right-click menu grows an "Open dagr" entry when a herdr-dagr plugin exposing the `open-dagr` action is installed and enabled. Detection reads the plugin registry only — never the filesystem — so an absent or disabled install simply means no entry: no greyed-out row, no error, nothing logged. `projects.yml` members may also declare a per-member `template:` that is substituted into `defaults.open_with`'s `<template>` placeholder (e.g. `herdr-plus open web`); when the named opener is not available, opening falls back to bora's own `workspace open` instead of failing. Neither dagr nor herdr-plus is ever a hard dependency.
...
- `COMMANDS` and `CHECKS` bands render under a worktree from the project's declared `sections:`, with a right-aligned `running/declared` count and a rule that fills to the counter column. No provider executes commands or collects check runs yet, so the counts read `0/N` and a project that declares no sections renders no bands at all.
