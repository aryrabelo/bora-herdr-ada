...
- Installers, release manifests and agent-detection manifests moved from `website/` to `distribution/`; `bora update` and the installers read `distribution/latest.json` / `distribution/preview.json`.
## [0.9.0] - 2026-09-07
...
- New `bora events` verb streams server events as JSON lines on stdout, one flushed line per event so piped consumers see each event the moment it happens. It subscribes to the 26 parameterless event kinds by default, takes `--subscribe <name>` (repeatable, wire names like `pane.created`), gates the three pane-scoped kinds (`pane.output_matched`, `pane.agent_status_changed`, `pane.scroll_changed`) behind `--pane <id>`, and exits 0 after `--limit <n>` events or on Ctrl-C. Streaming is the default behavior and `--follow` is accepted for compatibility with the ticket that named it; `--session <name>` streams from a named session.
- `channel.message` is subscribable. The server already emitted the event on every `#channel` append, and it already reached plugin hooks, but `events.subscribe` had no variant for it — so an external client could only learn about channel traffic by polling `channel tail --follow`, one process per room. `bora events --subscribe channel.message` now streams it, and it joins the default set, so a client that wants everything gets channel traffic too. The payload already carries the channel name, so a subscriber interested in one room filters on it rather than asking the server for a per-room subscription.
...
- The Project view sidebar mode (`ui.view_mode = "project"`) is retired — decided (ceo-bora#269/#270) ahead of the upstream herdr 0.9.0 sync rather than carried through it. It shipped in 0.45.5, so this is a breaking removal for anyone who adopted it: the `project.*` API verbs, `workspace.set_project`, the `projects.yml` store, the sidebar's project/section/pane-dots-band rendering, and the `[ui.sidebar.project]` config keys (`row_gap`, `glyph_style`) are all gone. `ui.view_mode` now cycles `flat` → `folders` → `repo`. **If your config still sets `view_mode = "project"`, edit it before updating:** the value is now an unknown variant, and at startup an unknown variant fails the whole document, so every other key falls back to its default until the line is changed (`bora config check` names it, and a running `server reload-config` degrades only `[ui]`). A session snapshot naming `"project"` restores as `repo` with its workspaces intact — `persist::snapshot` now falls back to the default view for any `view_mode` it does not recognize instead of failing the whole file, which previously discarded every workspace, tab and pane. An `[ui.sidebar.project]` table left behind is harmless — an unknown table is ignored with a diagnostic, not a parse failure. Folders — which stays the supported way to group workspaces by hand — is untouched.
- The sandbox orchestrator (`src/sandbox.rs`), the shared todo/scratchpad stores and their MCP tools, the right panel (Changes/Checks/Issues/PRs tabs, PR badges, collectible marker), the `[github]`/`[checks]`/`[flow]` background-polling config sections, and per-pane channel scope (`channel join --scope-write`/`--scope-read`, the `.scope.json` sidecar) are all retired — decided ahead of the upstream herdr 0.9.0 sync (ceo-bora#269/#271). `bora mcp serve`, `pane report-result`, `bora events`, and timed sidebar hide are unaffected. `prefix+shift+b` is freed (it was double-booked with `toggle_right_panel`, which is gone). The Create worktree modal drops its GitHub and Branch tabs — their data source (open-PR/issues/branches polling) no longer exists — and now goes straight to name entry. Events `todo.changed`, `scratchpad.changed`, `github.prs_refreshed`, `github.pr_opened`, and `github.issues_refreshed` no longer fire; `github.pulls.list`/`github.issues.list`/`todo.*`/`scratchpad.*` API verbs are gone. The channel protocol briefing is bumped to v6, so every agent pane already sitting in a `#channel` is re-briefed once after updating — a pane that had declared scope directories was told to confine itself to them, and that instruction is withdrawn.

### Fixed

- Creating a worktree no longer risks wiping the repo's `.git/info/exclude`. Bora appends a `.context/` line there, and it read the file with a lossy read whose failure it treated as "the file is empty" — so a single non-UTF-8 byte anywhere in the file (or a permissions error) made the follow-up whole-file write replace every ignore rule in it with just `.context/`. An unreadable exclude file is now left strictly alone. The write is also an append rather than a read-modify-write, so two bora processes creating worktrees in the same repo at once can no longer drop each other's lines; the worst case is a duplicate `.context/`, which git ignores just the same.
- A pane counted by the sidebar's `N waiting` label but carrying no agent — a plain shell promoted by the quiet-output tick — no longer renders identically to a seen one, so the dots you can see finally add up to the number the counter reports. Both its dot and its workspace name go plain white: the dot keeps the hollow ○ and the name skips the bold, a deliberately lesser weight than a finished agent, and yellow stays reserved for "agent finished, come read".
...
- Server mode froze every spinner. The headless tick never advanced the animation clock (`spinner_tick`) nor armed its timer — both lived only in the standalone app's tick — so in server mode, the mode most operators actually run, every working dot sat on one frame and idle age labels never counted. The decision now lives in one shared `App::tick_animation` helper (advance when due, then re-arm) called by both tick paths, so neither tick can drift from the other again.
...
- On the 16-colour `terminal` theme, `mauve` was the same grey as ordinary muted text, so every mauve accent — the worktree marker, the merged-PR chip — disappeared into the surrounding text; it now maps to the unclaimed ANSI purple slot. `surface0` was likewise identical to the sidebar background, so rows asking for a slightly lighter fill got none.

### Changed

- `bora channel send` delivers to agent members immediately, even when a recipient is mid-turn — the message arrives like steering and the receipt says `delivered`, instead of silently queueing until the pane next went idle. Two agents collaborating in one channel now hear each other during a turn, not after it. The old hold-until-idle behavior is an opt-in: `channel send --when-idle` (or `when_idle: true` on the wire) defers a Working target's delivery and reports it as `deferred` with its queue position. The channel protocol briefing was rewritten to teach the new default and bumped to v5, so already-briefed panes are re-briefed — and it now travels in the same delivery mode as the message, so an unbriefed busy pane reads the protocol before its first message.
...
- `.bora.toml`/`.bora/settings.toml` support is gone from core: the per-repo command menu (`[[commands]]`), worktree provisioning (setup script, file copy/symlink, `[ports]` allocation, `bora worktree create --no-setup` and the `setup` field in its `--json`), the `bora workspace run` subcommand, the Project view's COMMANDS band, and the per-repo `[flow]` command-template override all came from these two files, and all of it is removed end to end. Only the per-repo override goes: the global `[flow]` config in `config.toml` is untouched, so a repo that carried its own `[flow]` template in `.bora.toml` now falls back to that global template. Replaced by the plugin [`aryrabelo/bora-local-commands`](https://github.com/aryrabelo/bora-local-commands) (private repo) — clone it wherever you keep checkouts, then point bora at that clone with `bora plugin link <path-to-clone>`, which takes a filesystem path and nothing else. If your git credentials reach the repo, `bora plugin install aryrabelo/bora-local-commands` does the clone step for you.
