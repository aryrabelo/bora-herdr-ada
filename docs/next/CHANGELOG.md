...
- New `folders` sidebar view (`ui.view_mode = "folders"`, or cycle it with the view-mode toggle/`prefix+shift+v`): a flat list like Flat, but it honors your own `visual_group` folders and nothing else — no repo auto-grouping, no branch brackets, no `@wNpN` pane badge. Every workspace row carries the Project view's per-pane dots (one clickable ○/spinner per pane) instead of a single indicator. Assign a workspace to a folder with the workspace context menu, `bora workspace set-group`, or by dragging its row onto a folder header. A separate `ui.hide_pane_badges` toggle drops the synthetic `@wNpN` badge (a registered `bora agent rename` name is kept) in the other views.
