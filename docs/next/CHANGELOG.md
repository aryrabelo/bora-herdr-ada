...
- `bora channel send` delivers to agent members immediately, even when a recipient is mid-turn — the message arrives like steering and the receipt says `delivered`, instead of silently queueing until the pane next went idle. Two agents collaborating in one channel now hear each other during a turn, not after it. The old hold-until-idle behavior is an opt-in: `channel send --when-idle` (or `when_idle: true` on the wire) defers a Working target's delivery and reports it as `deferred` with its queue position. The channel protocol briefing was rewritten to teach the new default and bumped to v5, so already-briefed panes are re-briefed. Channel-name lookups also accept the name with or without the leading `#` in every verb, and an unknown channel answers with a named `channel_not_found` error instead of empty output.
