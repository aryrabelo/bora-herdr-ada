...
- Temporarily disabled the bora-flow (`[flow]`) agent-pane spawn after syncing upstream's agent-start redesign (spawn → attach), which removed the pane-spawn API it relied on. Running a flow now surfaces a "bora-flow unavailable" notice; re-porting it onto upstream's new pane primitives is tracked under `.local/prd/`.
- Bumped the server/client wire protocol to 19 so stale clients from either the bora or upstream protocol-18 lineage are cleanly rejected across this sync (both lineages independently minted a protocol 18 carrying different message sets).
