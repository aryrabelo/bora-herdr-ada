...
- CI now runs an independent-model review on every push to a PR (`.github/workflows/independent-review.yml`, driven by `scripts/independent_review.sh`): it diffs `base...head` so it reviews the pushed commit, uses a reviewer model from a different family than the one that authored the change (set via the `INDEPENDENT_REVIEW_MODEL` repo variable), and fails the job if the review could not run. Findings are advisory and do not block merge.
