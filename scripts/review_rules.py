...
def check_allow_justification(
    ...
    for path, added in added_lines_by_path.items():
        ...
        for lineno, text in added:
            ...
            # informational value. Gate the exemption on generated provenance (bindgen's
            # header line) so hand-written Rust keeps the strict rule.
            if (
                "clippy::unnecessary_operation" in text
                and "clippy::identity_op" in text
                and following.lstrip().startswith("const _: () = {")
                and head_lines
                and head_lines[0].startswith("/* automatically generated by rust-bindgen")
            ):
                continue
