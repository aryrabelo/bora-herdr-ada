...
class ReviewRulesEndToEnd(unittest.TestCase):
    ...
    def test_bindgen_shaped_allow_in_a_generated_file_is_clean(self) -> None:
        self.write(
            "src/bindings.rs",
        ...
        self.bump_version()
        self.commit("chore: refresh vendored bindings")
        self.assert_clean()

    def test_hand_written_allow_shaped_like_bindgen_is_flagged(self) -> None:
        # Same attribute shape, but the file is hand-written: no bindgen header,
        # so the exemption must not fire and the justification rule stays strict.
        self.write("src/lib.rs", "pub fn existing() {}\n")
        self.append(
            "src/lib.rs",
            "#[allow(clippy::unnecessary_operation, clippy::identity_op)]\n"
            "const _: () = { assert!(1 == 1); };\n",
        ...
        self.bump_version()
        self.commit("feat: add const assertion")
        self.assert_flags("allow")
