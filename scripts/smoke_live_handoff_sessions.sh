...
session_dir() {
  ...
  if [[ "$session" == "default" ]]; then
    printf '%s/bora-dev' "$CONFIG_HOME"
  else
    printf '%s/bora-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}
...
assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/bora-dev/herdr.sock | "$CONFIG_HOME"/bora-dev/sessions/*/herdr.sock)
      ;;
    *)
...
mkdir -p "$CONFIG_HOME/bora-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/bora-dev/config.toml"
