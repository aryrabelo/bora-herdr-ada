...
class AllowJustificationCheckTests(unittest.TestCase):
    ...
    def test_bindgen_shaped_allow_in_a_generated_file_is_exempt(self) -> None:
        head_lines = {
        ...
        added = {
            "src/ghostty/bindings.rs": [
                (3, "#[allow(clippy::unnecessary_operation, clippy::identity_op)]")
            ]
        }
        findings = check_allow_justification(added, head_lines)
        self.assertEqual(findings, [])

    def test_hand_written_file_with_the_bindgen_allow_shape_is_flagged(self) -> None:
        # The exemption exists for bindgen output; the same attribute shape in a
        # hand-written file (no bindgen header) must still carry a justification.
        head_lines = {
            "src/lib.rs": [
                "pub fn existing() {}",
                "",
