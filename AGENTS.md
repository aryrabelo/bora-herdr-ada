...
- **When something must become "runtime-configurable", find out whether the runtime axis is the TYPES or the INSTANCES before touching the type system.** T6 (bora-79l.10) asked for sidebar sections the user creates at runtime. A survey of the code named the blocker precisely and confidently: `WorkspaceListEntry::{SectionHeader,SectionItem}.kind` is `&'static SectionDescriptor`, `SectionPushCtx` is `Copy`, and `resolve_section_order` returns `[&'static SectionDescriptor; SECTION_COUNT]` sized off `REGISTRY.len()` — so a user-created descriptor "cannot satisfy `&'static` without leaking or an arena the process never frees". Every word of that is true and all of it was beside the point. The section TYPES are four and known at compile time (BRANCH/COMANDO/CHECKS/LIVRE); only the INSTANCES are born at runtime, and the instance type (`ui::sidebar::sections::Section`) already existed, already round-tripped through `projects.yml`, and was already read by the renderer (`project_view.rs`, `section_model_flags`). The whole feature is a `SectionKind` → const-descriptor map plus emission from `Project.layout`: no `Box::leak`, no arena, no variable-length array, no new `WorkspaceListEntry` variant — and therefore no cascade edit, which is how the bead's own "solve the generic-row problem first" gate was met by construction rather than by work. The generalisation: a demand for runtime configurability is usually a demand for runtime DATA, and the registry of kinds can stay `const`. Check which axis actually varies before paying for a dynamic one — the dynamic-descriptor version of this change would have added a leak and an allocation to a per-render × per-pane × per-client loop to express four values that never change. (learned 2026-08-28, binding.)
- **A menu item's label is written in one place and re-declared as a literal in each recognizer, and the failure mode is an item that renders and does nothing.** `section_control_menu_items` formats the labels, `section_toggle_field` and `section_create_kind` re-match them as string literals — three independent lists, no compiler tie. Rename one and the item still renders, still highlights, still closes the menu, and performs no action; nothing logs, and the operator reports "the menu is broken" with no way to say which half. The cheap net is a test asserting every emitted item is claimed by EXACTLY ONE recognizer (`^`, not `||` — double-claiming is the other desync), run across both spellings of every toggle: `every_emitted_item_is_claimed_by_exactly_one_recognizer` in `modal.rs`. Verified falsifiable by renaming one label and watching it redden. Same family as the `PLUGIN_HOOK_EVENT_KINDS` rule above — two lists that must agree, with silence as the symptom. (learned 2026-08-28, binding.)
- **In a sidebar rendering test, an entry's index is not its buffer row.** A new test read the rendered header at `WORKSPACE_LIST_TOP_MARGIN_ROWS + header_idx`, landed on a neighbouring workspace's line, and reported a production bug that did not exist — the name override was working the whole time. `PaneDotsRow` is two rows tall (and one row when `parts.dots` is off), and the list also honours `row_gap`, so the mapping from entry index to row is `entry_row_height`'s job, never arithmetic on the index. Summing the preceding heights is correct but still fragile against `row_gap`; scanning every rendered row for the string and asserting it appears exactly once is both simpler and stronger, and it still reddens when the logic is removed (verified by mutation). (learned 2026-08-28, binding.)
- **When several tests hardcode the same expectation, that expectation wants one owner — and it must be restated independently, not read from the code under test.** R3 gave the project header band a padding row per edge, and seven tests broke at once; four of them said the same thing in four places ("height 1, except `PaneDotsRow` is 2"). Breaking together is the signal the rule had no owner. `ui::sidebar::expected_entry_height` (`#[cfg(test)]`) is now that owner. The trap on the way there is subtle and was walked into once during the fix: the obvious implementation asks `project_band_top_pad` — the production helper — for the answer, which makes every consumer assert `f(x) == f(x)` and reddens for nothing. The neighbour walk is deliberately spelled out a second time in the test helper; two statements of one rule that must agree IS the test, and the mutation run confirms it (zeroing the pad in production reddens both the geometry test and `p4a`). Same shape as the sidebar three-pass lockstep rule above, one level up: there the three passes share a helper because they must agree with each other; here the test must NOT share it, because it must disagree when the helper is wrong. (learned 2026-08-28, binding.)
