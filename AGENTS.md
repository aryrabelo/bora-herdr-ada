...
- **On this machine `~/.config/bora/config.toml` is a symlink whose store entry points BACK OUT to `~/Sites/dotfiles-2026/dotfiles/bora/config.toml`, so it is WRITABLE — and that is more dangerous than read-only, not less.** Telling the operator to add a key to the former cost him a full home-manager switch that rebuilt the identical config and changed nothing, and the key path itself was correct — only the file was wrong. An earlier version of this rule concluded "read-only symlink into the nix store" and was measurably wrong about the consequence: `ls -l` shows the store path, but home-manager's `mkOutOfStoreSymlink` makes the store entry itself a link to the dotfiles checkout, so `readlink -f` lands in `~/Sites/dotfiles-2026` and `test -w` answers yes (measured 2026-09-17). Anything that follows the symlink and writes — a plugin's `configure --apply`, an editor, a `>>` redirect — silently edits the git-tracked dotfiles repo instead of failing, and the change then survives as an uncommitted diff in a repo nobody was looking at. So: resolve with `readlink -f` before instructing anyone to edit a dotfile here, edit the dotfiles source deliberately, and treat any third-party tool that offers to patch this file as a tool that wants commit access to your dotfiles. The same holds for `~/.config/ghostty/config`. There is also NO nix package for the bora BINARY, only its config and helper scripts, so `~/.local/bin/bora` from `just install` is the only binary on `PATH` (and `~/.local/bin/herdr` is a symlink beside it, so a tool hardcoding the upstream binary name still resolves) and a "stale version in dotfiles" is never the explanation.
...
- **A manual pane status is an override at read-out, never a second source of truth for detection (2026-09-14, binding).** `pane.set_status` stores `TerminalState::manual_status` (`ManualAgentStatus { status, set_at, set_seq }`, runtime-only, deliberately absent from session snapshots) and every place that reports a status folds it through the one helper, `app::api_helpers::effective_agent_status(manual, state, seen)`, which falls back to `pane_agent_status`: `pane_info`, `tab_info`, `workspace_info`, `terminal_target_candidate`, and the `pane.agent_status_changed` payload in `emit_pane_state_update`. Detection, hook authority, and the OSC-title reconcile are NOT gated by a pin — that is what makes clearing one reveal the live state on the next read instead of a stale cached value. Two consequences worth keeping: the tab/workspace rollups resolve competing pins by newest pin (`Tab`/`Workspace::manual_status_pin` order on `set_seq`, a process-wide monotonic counter — two pins stamped in one clock tick must still order by who was set second; `set_at` is diagnostics only), so the stamp must stay owned by `TerminalState::set_manual_status_at`; and `emit_pane_state_update` keeps gating emission AND the `when_idle` drain deadline on the DETECTED status while reporting the effective one, so a human pin cannot hold a queued prompt hostage. `unknown` is not settable (rejected with `invalid_params`, and the published `status` schema — `settable_agent_status_schema` — advertises only the four settable values plus `null`) — the override exists to state a human-meaningful status, and unknown is the absence of one. A new read-out of `agent_status` that calls `pane_agent_status` directly silently opts out of every pin. **Client side, the picker is a FLAT run of rows in the pane context menu, never a submenu:** this shell has no nested popup — the same constraint that made "move to group" a flat run of one row per path (ceo-bora#303) — so `ClientContextMenuItem::items()` appends borrowed-label rows after the existing pane items, FOUR settable statuses plus one un-pin (`Status: Working|Blocked|Idle|Done|Auto`, no `unknown` row because the server rejects it), carrying `ClientContextMenuAction::SetPaneStatus(Option<AgentStatus>)` with `Auto` = `None`, which `skip_serializing_if` sends as an absent `status` key. The menu is a stateless picker: no new field on `ClientContextMenuTarget::Pane`, no client-side echo of the chosen status — activation pushes one `Method::PaneSetStatus`, the overlay closes on the click, and the server's own `request_changes_ui` refresh repaints the row dot, which is the only feedback. Never wait on `pane.agent_status_changed` to close the menu or confirm the pick: re-picking the current status, or `Auto` on a pin that already matched detection, moves no EFFECTIVE status and so emits no event, only a plain `Ok`. Do not add a "current status" marker to the rows either; it would be a second, staler copy of a value the sidebar already renders.
- **A fallible send whose `Err` is discarded is a silent drop, and fourteen of them shipped before anyone measured the class (2026-09-17, ceo-bora#41, binding).** `try_send_bytes` (`src/terminal/runtime.rs`) returns a `Result` and thirteen of its fourteen production callers threw it away, so keystrokes from `terminal attach`/`agent attach`, both paste paths, the clipboard-image paste and the queued-prompt replay vanished under a momentarily full channel while every caller reported success. The queue that prevents this already existed — `PaneInputOverflow::send_or_queue` (`src/pane.rs`), reached through `send_bytes_preserving_order` — so each fix was a call swap, not new machinery. Three findings outlive the fix. **The fast path is only safe because of its guard:** `send_or_queue` takes it solely when `queue.is_empty() && !state.draining`, and `try_send_bytes` has no such guard, so "retry on `Err(Full)`" would push a paste AHEAD of keys already queued — trading a loss for a scramble, which is worse, because scrambled input looks plausible and a human never learns it happened. **A drop that logs nothing is invisible to log archaeology:** the server log carried 708 `WARN` and ZERO `dropping pane input`, which proved the server never dropped and pointed the search at the client; conversely `warn!(client_id, terminal_id, err = %err)` with no message string (`src/server/headless.rs`) made a real drop unfindable by `grep`. Every discard site names its condition in the message text. **Two ceilings derived from the same quantity without knowing about each other:** `MAX_INPUT_EVENT_BATCH = 4096` events (`src/server/client_transport.rs`) can become 4096 entries against `MAX_QUEUED_PANE_INPUT = 4096` entries (`src/pane.rs`), so one maximal client batch alone reaches the ceiling that exists for a wedged pane — filed, not fixed.
- **The input framer's tail-discard window was bounded by bytes and never by time, and it ate what the operator typed (2026-09-17, ceo-bora#41, binding).** An idle flush meeting an incomplete control sequence armed `discard_until` and cleared the buffer so a terminal reply could not leak into the pane as text (`src/raw_input.rs`); with no terminator ever arriving the window stayed armed and consumed typed bytes. Signature, measured live on the installed binary at ZERO output load: the burst loses a PREFIX and then recovers — every byte up to and including the first one outside `0x20..=0x3f`. Digits are all inside that range, so `23456789` lost 8 of 8; `abcdefgh` lost only the first letter, because `0x40..=0x7e` is the CSI final byte and is consumed AS the terminator. The `Osc` family discriminates nothing at all (the generic branch clears the buffer before testing plausibility), so there the whole idle interval is lost, letters included. Three corrections worth keeping: **output load is not the trigger** — a real client driven at 95,581 rendered lines/s lost none of 200 typed characters, and the 53 frames/s on the operator's screen had been read as causal; **no human typing speed avoids arming it**, because 30 ms/byte already exceeds `RAW_INPUT_IDLE_FLUSH_TIMEOUT_MS = 10`; and **the pre-existing test `host_reply_tail_discard_is_bounded_across_pushes` pinned the defect as a feature** — 131 digits in, 3 out — which a live pty reproduced byte for byte, so the behaviour was written down and green the whole time. The fix is the missing limit (expire at the next idle flush with nothing left to discard); a proposed "one grace flush" flag was rejected on measurement, since the client fires at most one extra flush per read (`src/client/input.rs`) and the grace would itself swallow the operator's first byte. Still unfixed and deliberately out of that scope: the catch-all at the end of `flush_timeout` clears the entire buffer, reachable only when a prefix and following bytes land in the SAME read (<10 ms: paste, macro, key repeat — never human cadence), one-shot and self-healing, and its own fix risks leaking mutilated sequences as text.
- **A deferred delivery receipt is not a delivery, and the stamp belongs on the queue's terminal fate (2026-09-17, ceo-bora#41, binding).** The channel protocol briefing was marked sent whenever `handle_agent_prompt` returned anything but a hard failure, so a `Deferred` prompt stamped the pane and then died on any of three paths — `capacity` eviction (`PENDING_AGENT_PROMPT_CAP = 32`, which kills the OLDEST entry, and a briefing is by construction the oldest), `fail_pending_agent_prompts` on pane close, and terminal replay failure (`agent_not_found`/`agent_not_ready`, i.e. exactly when the pane's agent CHANGED and the briefing matters most). A pane acting on a briefing it never received is worse than one missing a feature — the same asymmetry `CHANNEL_PROTOCOL_VERSION` exists to protect. The shape that fixes it: `Deferred` registers `queue_id -> pending`, and the stamp lands in `note_queued_prompt_delivered` / is cleared by `note_queued_prompt_dropped` (`src/app/api/channels.rs`), called from `report_queued_prompt_{delivered,dropped}` (`src/app/mod.rs`) AFTER `emit_event` — the event is the source of truth and the stamp its consequence; inverted, a panic between the two lines leaves a stamped pane with no event, which is the undebuggable state. Two properties make it sound rather than lucky: the replay preserves the ORIGINAL `queue_id` and `enqueued_at` on requeue (a fresh `enqueued_at` would REJUVENATE the entry and silently reorder capacity eviction), and `load_pending_agent_prompts` restores original ids while raising the counter past the highest, so id reuse — the only way a foreign prompt's fate could stamp an unbriefed pane — is impossible by construction. The in-memory pending map does not survive a restart; the orphan then no-ops and the pane is re-briefed, which is the same benign duplicate the 600 s `DEFERRED_BRIEFING_FATE_WINDOW` covers. That window is kept deliberately, now as a net for UNKNOWN paths: with it, the worst case is a duplicate briefing; without it, an unmeasured path leaks a pending that never stamps and re-briefs on every `channel.send`.
- **A test can be green because its fixture never reaches the assertion, and this cost two findings in one day (2026-09-17, binding).** `channel_protocol_survives_restart_without_resend` briefed synthetic pane ids (`"w1A:p2"`) that resolve to no runtime, so it measured delivery bookkeeping for a delivery that never happened; the sibling case in the fork's board was a delimiter assertion that passed while the string it examined came from a different file. Both read exactly like a characterization test of a contract. The discriminant that makes such a test honest is a **fixture guard that proves the failure occurs at the NAMED point** — e.g. assert the send failed inside `agent_prompt_failed` before asserting nothing was stamped — because without it the test passes for the wrong reason forever. Related and cheaper to catch: an assertion that restates what a locator already guaranteed is a tautology (`header_row` finds the row by its label, so `assert!(header.contains(label))` after it can never fail) and should be deleted rather than kept as reassurance.
- **A single failing run is not a bisection, and load is a confound that mimics an env-inheritance bug (2026-09-17, binding).** `restored_pane_shell_does_not_inherit_startup_cwd_env` (`tests/api_ping.rs`) failed inside a bora pane with `socket did not appear`, and a one-run-per-variable bisection accused `HERDR_BIN_PATH` — plausible, because this file already documents ambient `HERDR_*` inheritance breaking spawned-server tests. Measured properly, the flake is ~50% WITH the environment scrubbed too, nothing in the server even READS that variable (it is only written for children), and the real cause is a 5 s `wait_for_socket` deadline against a cold server start at load average 4–10 (produced by this session's own parallel agents). Rule: before attributing an intermittent failure to a variable, measure the rate with and without it — a 50% flake gives a clean-looking bisection on the first try, and the wrong mechanism would have been written into a permanent comment.
- **The fix traded an unbounded swallow of human typing for a bounded leak of a LATE host reply, and that trade is deliberate (2026-09-17, ceo-bora#41).** Measured live on 0.48.2 in an isolated lab: a genuine CSI tail arriving 1 ms after its prefix — the real-world shape, since the rest of a host reply lands in microseconds — is still SUPPRESSED (`leaked=''`), while a tail arriving 300 ms late now LEAKS its payload as text in the pane (`leaked='1n'`). The old binary would not have leaked it, because the window never expired on time; that half is INFERENCE from `discard_host_reply_csi_tail` and the bytes-only limit, not measurement, since the install replaced 0.48.0 and it can no longer be measured. The trade in one line: stop eating the operator's keystrokes indefinitely, accept that a host pathological enough to straddle an idle flush (10 ms) costs two characters of stray text. Do not "fix" the leak by widening the window back out in time; covering it properly means recognising the tail by shape rather than by a deadline.
- **An acceptance criterion can be logically impossible, and the number that looks like a regression is sometimes the only answer the code can give (2026-09-17, binding).** This lot shipped with a criterion that case (d) — armed window, then LETTERS typed after the idle interval — "must stay at 1/8 lost, because 0/8 would mean the window stopped closing on the genuine terminator". It cannot: (c) and (d) are the SAME condition, differing only in the character class typed AFTERWARDS, and the framer cannot distinguish a digit from a letter before the window is armed. Demanding (c)→0 and (d)→1 was asking for a behaviour that does not exist, and it very nearly became a "regression" in the commit message. The lesson is the substitution: a concern about protecting genuine host replies is not testable by typing letters, only by SENDING A GENUINE TAIL — which is why the two cases that actually settled it (1 ms tail suppressed, 300 ms tail leaked) had to be added after the fix. Before pinning an acceptance number, check that the two cases it contrasts are separable by the code under test at the moment the decision is made.
- **Reproducing a KEYBOARD defect needs a real pty client, and the harness must prove focus before any loss number counts (2026-09-17, binding).** Three traps, each of which independently produces a confident wrong number. **No CLI verb reaches the keyboard path**: `pane input` is right-click routing only, and `send-text`/`send-keys`/`run` all enter at `try_send_bytes` (`src/app/api/panes.rs`) with bytes ALREADY framed — so a CLI probe measures the send path and says nothing about `src/raw_input.rs`, no matter how convincing the output looks. **The working tree drifts mid-session**, so name a branch against the source of the INSTALLED commit and prove parity first (`git diff --stat <upstream-of-install> HEAD -- <file>` empty) — here the tree was +95 lines ahead of the running binary while the analysis was being written. **Client focus cannot be assumed**: on a clean config `ENTER` failed and a stray digit was consumed as a keybind that opened the settings overlay, silently turning typed bytes into UI actions that read exactly like input loss. The harness therefore proves focus with an 8/8 control probe before any measurement is accepted, and the surviving one for this defect lives at `/tmp/framer-repro-harness/` (9 cases, `BORA_BIN=... sh run.sh`) — rebuild it from this rule if it is gone, since a tmp path does not survive.
...
- **The PR/issue/kanban surface stays OUT of core: it is an external plugin fork, and the prior-art sweep that decided it is recorded here so nobody sweeps again (2026-09-17, binding).** This answers the open condition left by the right-panel tombstone above ("a real owner for the `gh` polling cost") and by the Project-view one: the owner asked for a Mastra-Factory-shaped board — area tabs at the top, a Review column of the week's PRs including merged and closed-without-merge, and cards carrying the linked issue — and the measured answer is that core gets **zero new lines** for it. Four shelves swept, with the empty searches named because a measured absence is a finding: (1) GitHub at large has NOBODY rendering a PR kanban by week in a terminal (`pull request dashboard terminal stars:>100` = 0, `"pull request" board tui stars:>10` = 0, `github issues tui stars:>300` = 0), and the obvious orchestrator candidate `vibe-kanban` (28.1k★) declares itself SUNSETTING in its own README; (2) the `herdr-plugin` topic's own docs settle the ceiling — "native non-terminal plugin UI are not part of plugin v1", placements are overlay/popup/split/tab/zoomed, so a plugin owns a pane or a tab and NEVER a sidebar band, while `placement = "tab"` is a real tab with a PTY reachable by all 121 API verbs; (3) `dlvhdr/gh-dash` (12.5k★, MIT) already covers the DATA (`is:merged merged:>={{ nowModify "-1w" }}`) and the ACTIONS (keybindings shell out with `{{.PrNumber}}`, i.e. the print's own "Open session" button) but its sections are TABS, never columns side by side, and it has no API to drive; (4) the UI in the owner's screenshots **is** Mastra Factory (`mastra-ai/mastra`, 28.1k★, Apache-2.0 except `ee/`), self-hostable, REST + CLI, and its `factory-bora[bot]` is the owner's own private GitHub App, NOT Factory.ai's `factory-droid[bot]`. Two local measurements closed it: a `node` process already answers on the Factory port here (`GET /web/factory/projects` → **401, not 404**, so the route exists and only wants a credential the agent does not have), and the area tabs the owner asked for ALREADY EXIST in this client — `render_tab_bar` with `ui.tab_bar_position = Top`, proven live by opening an installed plugin with `--placement tab` and watching the tab take the manifest's own title. The chosen vehicle is a fork of `virtualboard/herdr-virtualboard` (Rust-adjacent Go, MIT, kanban whose columns ARE the lifecycle and whose `d` dispatches an agent into a bora pane), at `aryrabelo/herdr-virtualboard`, taught to read the owner's REAL queue (`FIOS.md` + `gates/*.md` + `gh` PRs) instead of `vb` spec markdown. Do not re-port any of this into core — not the columns, not the PR cards, not a second navigation layer above the tab bar — without a new owner decision reversing this. The two facts that make the plugin route cheap, and that a future session should re-measure rather than assume: bora parses a plugin manifest's `min_herdr_version` with real semver (`src/app/api/plugins/manifest.rs:239`, `crate::update::Version::parse` + `required > current`), and the fork's own CLI surface has not diverged from upstream's — 26 command lines and all 23 `agent start --kind` values confronted with `bora <group> <verb> --help` on 0.48.0, zero argv changes needed.
