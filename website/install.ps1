...
if ($useLocalPackage -and $LocalPackageFormat -notin @("zip", "exe")) {
    throw "Local Bora package has unsupported format '$LocalPackageFormat'."
}
...
function Test-HerdrReleaseComplete {
    ...
    $herdrExe = Join-Path $ReleaseDir "bora.exe"
...
if ($useLocalPackage) {
    ...
    if (-not $channelWasExplicit) {
        if (-not [string]::IsNullOrWhiteSpace($existingBora)) {
            $detectedChannel = [string](& $existingBora channel show 2>$null | Select-Object -Last 1)
            $detectedChannel = $detectedChannel.Trim()
            if ($LASTEXITCODE -ne 0 -or $detectedChannel -notin @("stable", "preview")) {
                throw "Could not determine the existing bora update channel. Rerun with -Channel stable or -Channel preview."
            }
            $Channel = $detectedChannel
            Write-Step "Preserving existing bora $Channel channel"
    ...
    if ($null -eq $assetProperty -and
        ...
        Write-Step "Fetching bora preview manifest"
        $manifest = ConvertTo-ManifestObject -Manifest (Invoke-RestMethod -Uri $ManifestUrl)
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($ExpectedBuildId) -and [string]$manifest.build_id -ne $ExpectedBuildId) {
        throw "Preview manifest changed while updating. Expected build $ExpectedBuildId but found $($manifest.build_id). Run bora update again."
    }
    ...
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-HerdrReleaseComplete -ReleaseDir $releaseDir -Format $asset.Format)) {
            $downloadPath = if ($useLocalPackage) {
                ...
                Join-Path $tempDir "bora-download.$($asset.Format)"
            }
            ...
            if (-not (Test-HerdrReleaseComplete -ReleaseDir $stagingDir -Format $asset.Format)) {
                throw "Downloaded bora package is incomplete or failed ConPTY verification."
            }
            $stagedHerdr = Join-Path $stagingDir "bora.exe"
            & $stagedHerdr --version *> $null
            if ($LASTEXITCODE -ne 0) {
                throw "Downloaded bora command failed verification: $stagedHerdr --version"
            }
            $backupDir = $null
            ...
            try {
                ...
                Write-WarningStep "Windows could not activate the downloaded release. Another process may have a package file open, such as antivirus or indexing. No incomplete release was activated. Run bora update again."
                throw
            }
        }

        $releaseHerdr = Join-Path $releaseDir "bora.exe"
        & $releaseHerdr --version *> $null
        if ($LASTEXITCODE -ne 0) {
            throw "Installed bora command failed verification: $releaseHerdr --version"
        }
